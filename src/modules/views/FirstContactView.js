...
const FirstContactView = {
  ...
  _animateRankingArrangement(stage, avatars, maxAbility) {
    ...
    // Get stage performances and sort by ability (overall ranking regardless of tribe)
    const stagePerfs = this.context.survivorStagePerformances[stage.id] || [];
    ...
    // Create a map of survivors to their performance data for easy lookup
    const survPerformanceMap = new Map();
    stagePerfs.forEach(perf => {
      survPerformanceMap.set(perf.survivor.firstName, perf);
    });

    // Sort all avatars by their performance ability (highest first)
    ...
    console.log('Overall ranking order:', sortedAvatars.map((a, i) => 
      `${i + 1}. ${a.survivor.firstName} (${(a.survivor._fc_ability || 0).toFixed(2)})`
    ));

    // Calculate container properties for centered ranking
    const containerWidth = this.container.clientWidth;
    const containerHeight = this.container.clientHeight;
    const centerX = containerWidth / 2 - 25; // Center horizontally (avatar width is 50px)
    
    // Calculate vertical spacing that fits within viewable area
    const topMargin = containerHeight * 0.1; // 10% from top
    const bottomMargin = containerHeight * 0.1; // 10% from bottom 
    const availableHeight = containerHeight - topMargin - bottomMargin;
    const spacing = Math.min(70, availableHeight / Math.max(1, sortedAvatars.length - 1));
    ...
    sortedAvatars.forEach((avatarData, overallRank) => {
      ...
      const targetY = topMargin + (overallRank * spacing);
      ...
      setTimeout(() => {
        ...
        // Calculate translation needed to center and position vertically
        ...
        avatar.style.zIndex = 1000 + overallRank; // Ensure proper layering
        
        // Remove any existing ranking badges
      ...
      }, overallRank * 150); // Stagger animations by 150ms each
    });

    // Calculate total animation time
    const totalRankingTime = (sortedAvatars.length * 150) + 1000 + 500; // Stagger time + transition time + buffer
