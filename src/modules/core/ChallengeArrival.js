import { tribeDisplayColor } from './LastFlagField.js';
...
export function buildChallengeArrival({ day, tribes = [], survivors = [], seasonHistory = [], tribalHistory = [] }) {
  ...
  const previous = returning && named ? {
    ...
    tribeColor: tribeDisplayColor(returning),
  ...
  const ordered = previous
    ...
    : activeTribes;
  const arrival = tribe => ({
    key: keyOf(tribe), name: nameOf(tribe), color: tribeDisplayColor(tribe),
  ...
  beats.push(
    ...
    { title: 'The rules', text: activeTribes.length === 3
      ? 'Twenty-one flags. All three tribes rotate turns, one contestant at a time. Take one, two, or three. The tribe taking the final flag loses. The other two win immunity.'
  ...
  beats.push(
    { title: 'LAST FLAG', text: activeTribes.length === 3 ? 'Do not be the tribe left with the last flag.' : 'The last flag wins immunity.', label: 'JEFF', titleCard: true, action: 'Take your spots' },
    { title: 'Survivors Ready?', text: 'These are your lineups. Stay sharp. Survivors ready?', label: 'JEFF', lineup: true, action: 'Start Last Flag' }
  );
